...
public:
	...
	void Hook()
	{
		...
		if (Interfaces.pClient)
		{
			...
			oCreateMove = VMTClient.dwHookMethod(reinterpret_cast< DWORD >(&Hooked_Createmove), 22);
		...
		{
			VMTSurface.bInitialize((DWORD**)Interfaces.pSurface);
			oLockCursor = (LockCursor)VMTSurface.dwHookMethod((DWORD)&Hooked_LockCursor, 67);
		}
